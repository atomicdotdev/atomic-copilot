...
Write code and iterate. You do **not** create or switch views, and you do **not** run `atomic add` or `atomic record` — the Copilot integration's hooks own all of that:
...
- Use `atomic vault sync` after editing any vault markdown files, and before every `intent show`/`update` — otherwise the CLI reads stale state and `update` clobbers your edits
